...
echo "Fetches Linux variants from download.switchroot.org - Version 1.0.0"
...
# Add a manual local file option
OPTIONS+=("0) Use local OS image file (.7z)")
MAX_RETRIES=3
...
echo 
while true; do
    read -p "$(tput bold)Enter number (0-${#OPTIONS[@]}): $(tput sgr0)" DISTRO_CHOICE
    if [ "$DISTRO_CHOICE" = "retry" ]; then
        echo "Refreshing selection..."
        continue
    fi
    if [[ "$DISTRO_CHOICE" =~ ^[0-9]+$ ]] && [ "$DISTRO_CHOICE" -ge 0 ] && [ "$DISTRO_CHOICE" -le "${#OPTIONS[@]}" ]; then
        if [ "$DISTRO_CHOICE" -eq 0 ]; then
            # User wants to supply a local file
            while true; do
                read -e -p "Enter full path to .7z OS image: " LOCAL_OS_PATH
                if [ -f "$LOCAL_OS_PATH" ]; then
                    ZIP_FILE=$(basename "$LOCAL_OS_PATH")
                    cp "$LOCAL_OS_PATH" "$TEMP_DIR/$ZIP_FILE"
                    if [ $? -ne 0 ]; then
                        echo "Error: Failed to copy $LOCAL_OS_PATH to $TEMP_DIR!"
                        exit 1
                    fi
                    echo "  Local file copied to $TEMP_DIR/$ZIP_FILE."
                    DISTRO="local_os"
                    ZIP_URL="file://$LOCAL_OS_PATH"
                    ID="LOCAL_OS"
                    PREFIX="/switchroot/local_os/"
                    INI="local_os.ini"
                    break 2
                else
                    echo "File does not exist or is not accessible. Please try again."
                fi
            done
        else
        ...
        echo "  Error: Invalid choice! Enter a number between 1 and ${#OPTIONS[@]} or 'retry'."
    fi
done
...
# Patch: If using a local OS image, auto-detect the extracted switchroot subdirectory
if [ "$DISTRO" = "local_os" ]; then
    # Find the first subdirectory under $TEMP_DIR/switchroot (excluding 'install')
    DETECTED_DIR=$(find "$TEMP_DIR/switchroot" -mindepth 1 -maxdepth 1 -type d ! -iname 'install' | head -n 1)
    if [ -n "$DETECTED_DIR" ]; then
        DISTRO_LOWER=$(basename "$DETECTED_DIR")
        echo "  [Auto-detect] Using extracted folder: $DISTRO_LOWER"
    else
        echo "  Error: No extracted switchroot subdirectory found!"
        exit 1
    fi
fi
...
if [ "$DISTRO" = "local_os" ]; then
    FINAL_DISTRO_NAME="$DISTRO_LOWER"
else
    FINAL_DISTRO_NAME="$DISTRO"
fi
echo "  To boot into $FINAL_DISTRO_NAME:"
echo "    1. Unplug the USB cable."
echo "    2. In Hekate, select '$FINAL_DISTRO_NAME' from 'More Configs' menu."